...
class PhantomBot(object): 
    command = "/usr/local/bin/phantomjs --proxy=127.0.0.1:%s --proxy-type=socks5 ./phantom.example.js"
    ...
    def _run_phantomjs(self):
        self.phantomjs = Popen(self.command_args, stdout=sys.stdout, stderr=sys.stderr)
